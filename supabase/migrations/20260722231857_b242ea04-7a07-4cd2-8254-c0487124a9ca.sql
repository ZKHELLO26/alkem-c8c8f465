ALTER FUNCTION public.record_public_scan(jsonb) SECURITY INVOKER;

GRANT SELECT, INSERT, UPDATE ON public.scan_users TO anon;
GRANT SELECT, INSERT ON public.scan_submissions TO anon;
...
CREATE POLICY public_scan_rpc_read_users
ON public.scan_users FOR SELECT TO anon
USING (current_setting('app.record_public_scan', true) = 'on');

CREATE POLICY public_scan_rpc_insert_users
ON public.scan_users FOR INSERT TO anon
WITH CHECK (current_setting('app.record_public_scan', true) = 'on');

CREATE POLICY public_scan_rpc_update_users
ON public.scan_users FOR UPDATE TO anon
USING (current_setting('app.record_public_scan', true) = 'on')
WITH CHECK (current_setting('app.record_public_scan', true) = 'on');

CREATE POLICY public_scan_rpc_read_submissions
ON public.scan_submissions FOR SELECT TO anon
USING (current_setting('app.record_public_scan', true) = 'on');

CREATE POLICY public_scan_rpc_insert_submissions
ON public.scan_submissions FOR INSERT TO anon
WITH CHECK (current_setting('app.record_public_scan', true) = 'on');
